...
figura_consumo_livre.update_layout(title=dict(    #Função dict atribui uma série de caracteristicas a variável(dicionário)
    ...
    font_color="#07325a" #Cor do título
),
...
app.layout = html.Div(children=[                                                        
    html.H1(children= 'Panorama da Energia Elétrica no Brasil', style = {'background':'#07325a', 'color':'#ffffff',"text-align":"center", }), #Título do gráfico
        dcc.Dropdown(
            id = "classe_geracao_consumo", #Primeiro gráfico
            options = [{'label': regiao, 'value': regiao} for regiao in regioes], #Passa pelas labels
            value = "Norte", #Inicia com norte
            clearable = False
            ),
        dcc.Graph(id = "fig_geracao_consumo"),#O tipo de saída gráfica
        
    
    html.Hr(), #Divisão física
    html.Div([
        html.Div([dcc.Graph(figure = figura_consumo_livre), #Segundo gráfico
            ], style = {'width':'40%'}), #Saída gráfica consumo livre, ocupação de 40% a esquerda
        html.Div([				    						   		                                                                       
        dcc.Dropdown(
            id='classe_consumo_pib', #Terceiro gráfico
                    options=[ #Passa pelas labels
                    {'label': 'Norte', 'value': 'Nor'},
                    {'label': 'Nordeste', 'value': 'Nord'},
                    {'label': 'Sudeste', 'value': 'Sd'},
                    {'label': 'Sul', 'value': 'Sl'},
                    {'label': 'Centro-Oeste', 'value': 'CO'}],
                value='Nor'#Inicia com norte
                ),
            dcc.Graph(id='fig_consumo_pib'), 
        ],style = {'width':'60%'}),#Saída gráfica consumo vs PIB, ocupação de 40% a direita
    ...
    html.Div([
        html.Div([
        dcc.Dropdown(
            id='classe_tarifa_media',#Quarto gráfico
            options=[#Passa pelas labels
            ...
            value='2012',#Inicia com 2012
            ),
        dcc.Graph(id='fig_tarifa_media'),
        ], style={'display': 'inline-block','width': '50%', 'float': 'left'}),#Saída gráfica tarifa media, ocupação de 50% a esquerda
        
        html.Div([                                                                         
        dcc.Dropdown(
            id='classe_luz_para_todos',    #Quinto gráfico                                                 
            options=[#Passa pelas labels
                {'label': '2012', 'value': '2012'},                                        
                {'label': '2013', 'value': '2013'},                                         
                {'label': '2014', 'value': '2014'},                                         
        ...
        ], style={'width': '50%', 'float': 'right'}),   #Saída gráfica luz para todos, ocupação de 50% a direita       
    ])
])
...
def luz_para_todos(year):
    ...
    figura_luz_para_todos.update_layout(title = dict(
                                text='Programa Luz Para Todos no Ano de {}\n [por mil habitantes]'.format(year),#.format responsável por associar o {} ao número de acordo com as labels
                                font=dict(size=18),#Determina o tamanho da letra
                                xref='paper', # Área central do gráfico
                                yref='container', # Área externa ao paper
                                x=0.5, # Faz o posicionamento horizontal do texto de acordo com o xref
...
def tarifa_media(anos_tarifa_media):
    ...
    barra = go.Bar(x= anos,# eixo x, em anos
                ...
                marker={'color': '#2178bb'}) # Cor dos gráficos   
    #Título do gráfico 
    config = go.Layout(title = dict(
                                text='Tarifa Média por Região no Ano de {} [R$/MWh]'.format(z),#.format responsável por associar o {} ao número de acordo com as labels
                                font=dict(size=18),#Determina o tamanho da letra
                                xref='paper', # Área central do gráfico
                                yref='container', # Área externa ao paper
                                x=0.5, # Faz o posicionamento horizontal do texto de acordo com o xref
...
def consumo_pib(local):
    ...
    figura_consumo_pib.add_trace(
        go.Scatter(x=anos, y=valor_consumo, name="Consumo [MWh]",line = {'color': '#07325a'}),
        secondary_y=False,                                 #Determina uma linha como False para a fig de dois y
    )

    figura_consumo_pib.add_trace(
        go.Scatter(x=anos, y=valor_pib, name="PIB [R$]",line = {'color': '#2178bb'}),
    ...
    figura_consumo_pib.update_layout(font_color="#07325a")                  #Cor da legenda 
